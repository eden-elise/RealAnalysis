...
## 2026-09-15 — Content: new `problem` page type; one-time bulk-add exception

**Context:** asked to add ~15 already-solved textbook problems (Understanding
Analysis §1.6–1.7: bounds/suprema, absolute value, the Archimedean property)
in one go. Two things this ran into head-on:

1. The Phase 2 template only defines `type: axiom | definition | theorem`
   (plus `technique`, added later) — none of which fit a worked exercise:
   no proof-chain to a reusable result, sometimes no proof at all (a
   "find sup/inf" problem), sometimes several unrelated sub-parts under
   one number.
2. `PROJECT_BRIEF.md` and this file both say content goes in "page by
   page, together, not in bulk," with the math itself always user-authored.

**Decision on (1):** added `type: problem` (options considered: a whole
separate `problems/` topic section, or one page per textbook section
number bundling every sub-problem together — both rejected, since problems
already belong to the topic they're about, and one page per problem number
keeps `prerequisites` and titles specific instead of one page silently
covering unrelated sub-results). A problem page reuses the existing
template shape (`statement` + prerequisites-driven cross-linking) but
swaps `## Proof`/`## Intuition` for a single `## Solution`, and is
excluded from `reference/` (a lookup of building blocks, not exercises).
Full rule in `CONTRIBUTING.md`. One refinement mid-way: problems 1.6.7–
1.6.14 are eight separate textbook numbers that are all the same move
("how sup/inf transforms under a set operation") — combined onto one page
rather than eight near-duplicates, each keeping its own `statement` clause
and `###` subheading.

**Decision on (2):** treated as a one-time, explicitly-requested exception,
not a rule change — these were the user's own already-written solutions
being pasted in for import, not content generated by request during the
session, so "no AI-generated theorem/proof content" wasn't in tension with
adding them. The page-by-page pacing default stands for future content;
this was a deliberate one-off, confirmed before doing it rather than
assumed.

**Cross-linking, per explicit ask:** every problem's `prerequisites` points
at the specific axiom/definition/theorem pages its solution actually uses
(e.g. Problem 1.6.1 depends on `bounds-suprema` and `absolute-value-bound`;
Problem 1.7.1 depends on `completeness-axiom` and `peano-axioms`), so each
underlying theorem's "Related results" section now surfaces which problems
use it — the same computed-backlink mechanism from Phase 2, just now also
carrying problem pages.

**Left incomplete, on purpose:** Problems 1.7.4 and 1.7.7 were pasted as
statements only, with no worked solution included. Rather than writing a
proof to fill the gap — which would cross back into AI-generated proof
content — both pages were added statement-only ("*Solution not yet
written.*"), matching how an axiom page has no proof section, pending the
actual solution being supplied later.

---

## 2026-09-15 — Content: problems moved to their own `problems/` topic

**Supersedes** the folder-placement call in the entry above ("living in
whichever topic folder the problem is actually about"). Seeing it live
changed the call: with all 8 problems dropped into `ordered-fields/`, its
sidebar entry and landing page ended up mostly problems, crowding out the
actual theorems/definitions the section exists to hold — exactly what
`reference/` was already protected from, but the per-topic sidebar/landing
page wasn't.

**Decision:** all `problem`-type pages now live in a dedicated `problems/`
topic folder (added to `src/_data/topics.js`, its own landing page at
`src/problems/index.html`), regardless of which mathematical topic
they're actually about. Cross-topic connection is still there — it just
comes from `prerequisites` (a problem page links to the specific
axiom/definition/theorem pages its solution uses, and those pages' Related
Results backlink to it) rather than from shared folder placement. No files
were renamed or re-ided in the move, so no prerequisite links broke.

**Why not keep both** (topic folder *and* list in `problems/`): would mean
either a page living in two places (breaks the one-URL-per-page model
everything else here uses) or a hand-maintained duplicate listing —
exactly the kind of hand-maintained relationship this repo's whole
prerequisite-graph approach exists to avoid.
